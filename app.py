...
def analytics_ftx_slippages():
    ...
    entries = list(map(dict, state.execute("select timestamp, market, order_size, buy_slippage_bps, sell_slippage_bps, total_slippage_bps from slippages order by market, order_size")))
