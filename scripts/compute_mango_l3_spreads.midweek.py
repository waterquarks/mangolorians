...
def main():
    ...
    db.execute("""
        insert or replace into competitors values
            ...
            ('2Fgjpc7bp9jpiTRKSVSsiAcexw8Cawbz7GLJu8MamS9q', 'BTC-PERP', 25000),
            ('2Fgjpc7bp9jpiTRKSVSsiAcexw8Cawbz7GLJu8MamS9q', 'ETH-PERP', 25000),
            ('2Fgjpc7bp9jpiTRKSVSsiAcexw8Cawbz7GLJu8MamS9q', 'SOL-PERP', 25000),
            ('2Fgjpc7bp9jpiTRKSVSsiAcexw8Cawbz7GLJu8MamS9q', 'AVAX-PERP', 25000),
            ('2Fgjpc7bp9jpiTRKSVSsiAcexw8Cawbz7GLJu8MamS9q', 'LUNA-PERP', 25000)
    """)
    ...
    uptimes = list(db.execute("""
            with
                params("from", "to") as (
                    values ('2022-05-13 12:00:00', strftime('%Y-%m-%d %H:00:00', datetime(current_timestamp)))
                ),
                ticks as (
                    select
                        market,
    ...
    cur.execute('drop table if exists midweek_competitors')

    cur.execute('create table if not exists midweek_competitors (market text, account text, target_depth integer, target_spread float, target_uptime float, uptime_with_target_spread float, uptime_with_any_spread float)')

    print(cur.query.decode('utf-8'))

    psycopg2.extras.execute_values(cur, "insert into midweek_competitors values %s", uptimes)
